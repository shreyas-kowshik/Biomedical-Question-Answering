...
EXPERIMENTS_DIR = os.path.join(MAIN_DIR, "train_factoid") # relative path of experiments dir
...
def initialize_model(session, model, train_dir, expect_exists):
    ...
    model.saver.restore(session,'train_factoid/qa.ckpt-65000')
    print('\n\n\n\n\nLOADED\n\n\n\n\n')
...
def main(unused_argv):
    ...
    train_context_path = os.path.join(FLAGS.data_dir, "train_factoid.context")
    train_qn_path = os.path.join(FLAGS.data_dir, "train_factoid.question")
    train_ans_path = os.path.join(FLAGS.data_dir, "train_factoid.span")
    dev_context_path = os.path.join(FLAGS.data_dir, "dev_factoid.context")
    dev_qn_path = os.path.join(FLAGS.data_dir, "dev_factoid.question")
    dev_ans_path = os.path.join(FLAGS.data_dir, "dev_factoid.span")
    ...
    if FLAGS.mode == "train":
        ...
        with tf.Session(config=config) as sess:
            ...
            _, _ = qa_model.check_f1_em(sess, train_context_path, train_qn_path, train_ans_path, "dev", num_samples=10, print_to_screen=True)
